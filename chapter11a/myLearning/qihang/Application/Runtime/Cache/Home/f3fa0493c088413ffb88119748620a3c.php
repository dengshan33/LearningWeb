...
<section id="1">
    ...
    <div style="margin-left: 45%; margin-top: 10px;">
        <a href="<?php echo U('Borrow/borrow');?>" style="font-size: 20px; text-decoration: none;">课程管理(点击报名课程)</a>
    </div>
    <div class="container">
        <div class="raw">
            <div class="col-md-12">
                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vol): $mod = ($i % 2 );++$i; if($vol["username"] == $username): ?><p>
                        <font class="name" style="color: #46b8da;">课程名：<?php echo ($vol["bookname"]); ?></font>
                        <font class="name" style="color: #46b8da;">课程编号：<?php echo ($vol["book_id"]); ?></font>
                        <font class="addtime"  style="color: black; margin-left: 15%;">报名时间：<?php echo ($vol["addtime"]); ?></font>
                        <br/>
                        <font class="content"  style="color:green;">课程时长：<?php echo ($vol["addtime"]); ?></font>
                    </p><?php endif; ?>
